defmodule TextingWeb.Dashboard.RecipientController do
  ...
  def add(conn, %{"recipients_list" => recipients_params}) do
    ...
    case Sales.add_to_recipients(recipients, recipients_params) do
      {:ok, _order} ->
        message =
          "#{count} contacts added to recipients list! Remember, Duplicate phone number will not be added."
  ...
  def show(conn, _params) do
    ...
    list_line_items =
      from(r in Texting.Sales.Order,
        where: r.id == ^recipients.id and r.status == "In Recipients List",
        select: fragment("unnest(line_items)")
      )
  ...
  def delete(conn, %{"id" => id}) do
    ...
    case Sales.delete_recipient(recipients, id) do
      {:ok, recipients} ->
        ...
        |> redirect(
          to: recipient_path(conn, :show, recipients: recipients, phonebook_id: phonebook_id)
        )
